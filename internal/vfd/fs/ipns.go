...
func GetFileSystem(ctx context.Context, path string) (FileSystem, error) {
	c, err := local.GetIPFSClient()
	if err != nil {
		return nil, err
	}
...
func SaveFileSystem(ctx context.Context, fs FileSystem) error {
	// Call the IPFS client to get the UnixFS API.
	c, err := local.GetIPFSClient()
	if err != nil {
		return err
...
func PublishFileSystem(ctx context.Context, fs FileSystem) error {
	// Call the IPFS client to get the UnixFS API.
	c, err := local.GetIPFSClient()
	if err != nil {
		return err
